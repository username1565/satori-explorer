...
function getReadableCoins(coins, digits, withoutSymbol) {
    ...
    var fullAmount = parseInt(coins || 0) / coinUnits;
    var amountDollars = Math.floor(fullAmount);
    var amountCents = ""; //.substr(0, (digits || coinDisplayDecimals));
    var fullAmountStr = "" + fullAmount;
...
function routePage(loadedCallback) {
    ...
    if (!page) {
        page = $link.selector.substr($link.selector.indexOf("#") + 1, $link.selector.length - $link.selector.indexOf("#") -3) + ".html";
    }
    xhrPageLoading = $.ajax({
        url: 'pages/' + page,
        cache: false,
